...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
       return await query.answer("മറ്റുള്ളവർ 𝐬𝐞𝐚𝐫𝐜𝐡 ആക്കിയ ലിസ്റ്റ്ല്\nകൈ ഇടാതെ സ്വന്തമായി 𝐬𝐞𝐚𝐫𝐜𝐡\nചെയ്തിട്ട് അതിൽ നോക്കുക👹", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("𝐘𝐨𝐮 𝐚𝐫𝐞 𝐮𝐬𝐢𝐧𝐠 𝐭𝐡𝐢𝐬 𝐟𝐨𝐫 𝐨𝐧𝐞 𝐨𝐟 𝐦𝐲 𝐨𝐥𝐝 𝐦𝐞𝐬𝐬𝐚𝐠𝐞, 𝐩𝐥𝐞𝐚𝐬𝐞 𝐬𝐞𝐧𝐝 𝐭𝐡𝐞 𝐫𝐞𝐪𝐮𝐞𝐬𝐭 𝐚𝐠𝐚𝐢𝐧..",show_alert=True)
        return
    ...
    if n_offset == 0:
        ...
        btn.append([InlineKeyboardButton(f"🔰 {round(int(offset)/10)+1} / {round(total/10)}", callback_data="📚 𝐏𝐚𝐠𝐞𝐬"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ⏩", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
...
async def advantage_spoll_choker(bot, query):
    ...
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("<b><i>മറ്റുള്ളവർ 𝐬𝐞𝐚𝐫𝐜𝐡 ആക്കിയ ലിസ്റ്റ്ല്\nകൈ ഇടാതെ സ്വന്തമായി 𝐬𝐞𝐚𝐫𝐜𝐡\nചെയ്തിട്ട് അതിൽ നോക്കുക👹</i></b>", show_alert=True)
    ...
    if k==False:
        ...
        if files:
            ...
            k = await query.message.edit('𝐓𝐡𝐢𝐬 𝐌𝐨𝐯𝐢𝐞 𝐍𝐨𝐭 𝐅𝐨𝐮𝐧𝐝 𝐈𝐧 𝐃𝐚𝐭𝐚𝐁𝐚𝐬𝐞 𝐨𝐫 𝐈𝐭 𝐢𝐬 𝐍𝐨𝐭 𝐑𝐞𝐥𝐞𝐚𝐬𝐞𝐝')
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            if (st.status == "creator") or (str(userid) in ADMINS):
                ...
                await query.answer("𝐓𝐡𝐚𝐭𝐬 𝐧𝐨𝐭 𝐟𝐨𝐫 𝐲𝐨𝐮!!",show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('✖️ 𝐁𝐚𝐜𝐤', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('✖️ 𝐁𝐚𝐜𝐤', callback_data='manuelfilter')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('✖️ 𝐁𝐚𝐜𝐤', callback_data='extra')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"🔰 1/{round(int(total_results)/10)}",callback_data="📚 𝐏𝐚𝐠𝐞𝐬"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ⏩",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
    ...
    if imdb:
        ...
        cap = f"𝐇𝐞𝐫𝐞 𝐢𝐬 𝐰𝐡𝐚𝐭 𝐢 𝐟𝐨𝐮𝐧𝐝 𝐟𝐨𝐫 𝐲𝐨𝐮𝐫 𝐫𝐞𝐪𝐮𝐞𝐬𝐭 {search}"
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("𝐈 𝐜𝐨𝐮𝐥𝐝𝐧'𝐭 𝐟𝐢𝐧𝐝 𝐚𝐧𝐲 𝐦𝐨𝐯𝐢𝐞 𝐢𝐧 𝐭𝐡𝐚𝐭 𝐧𝐚𝐦𝐞.")
    ...
    await msg.reply("𝐈 𝐜𝐨𝐮𝐥𝐝𝐧'𝐭 𝐟𝐢𝐧𝐝 𝐚𝐧𝐲𝐭𝐡𝐢𝐧𝐠 𝐫𝐞𝐥𝐚𝐭𝐞𝐝 𝐭𝐨 𝐭𝐡𝐚𝐭\n𝐃𝐢𝐝 𝐲𝐨𝐮 𝐦𝐞𝐚𝐧 𝐚𝐧𝐲 𝐨𝐧𝐞 𝐨𝐟 𝐭𝐡𝐞𝐬𝐞?\nനിങ്ങൾ ഉദ്ദേശിച്ച മൂവി താഴെ കാണുന്ന വല്ലോം ആണ് എങ്കിൽ.\nഅതിൽ ക്ലിക്ക് ചെയ്യുക", reply_markup=InlineKeyboardMarkup(btn))
